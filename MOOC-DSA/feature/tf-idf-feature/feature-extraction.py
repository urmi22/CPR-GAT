...
def build_concept_feature(concepts, gm):
	...
	# unique_concepts = list(concept_feature.keys())
	...
	print("Shape of concept-node feature {}".format(mat.shape))
...
def build_doc_feature(docs, concepts, cf):
	...
	for i, doc in enumerate(docs):
		...
		for concept in concepts:
			...
			if concept in doc_lemma:
				tmp_list.append(concept)

		if len(tmp_list) != 0:
			v = [cf[item] for item in tmp_list]
			doc_feature[doc_lemma] = np.average(v, axis = 0)

	print("Document feature extracted!")
	unique_docs = list(doc_feature.keys())
	doc_to_index = doc_index(unique_docs)

	doc_feature_matrix = np.array([doc_feature[i] for i in unique_docs])
	mat = np.matrix(doc_feature_matrix)

	print("Shape of document-node feature {}".format(mat.shape))
	...
	return unique_docs, doc_feature


def tfidf(docs, concepts):
	
	pipe = Pipeline([('count', CountVectorizer(vocabulary = concepts)),('tfid', TfidfTransformer())]).fit(docs)
	tf = pipe['count'].transform(docs).toarray()
	idf = pipe['tfid'].idf_
	tmp = np.tile(idf, (len(docs), 1))
	TfIdf = tf * tmp
	...
	print("Shape of document-concept-edge feature {}".format(mat.shape))
...
def cos_sim(docs, df):
	...
	print("Shape of document-document-edge feature {}".format(mat.shape))
...
def pmi(docs, concepts):
	...
	# print("minimun doc length: {}".format(min_doc_length))

	window_size = 10
	concept_pairs = []
	count = {}
	concept_to_concept = np.zeros((len(concepts), len(concepts)))
	# concept_to_concept = np.zeros((4,4))

	# for i in range(len(concepts)):
	# 	for j in range(i+1, len(concepts)):
	# 		concept_pairs.append(tuple((concepts[i], concepts[j])))

	# print("concept-pairs created!")


	# windows = [doc[i : i + window_size] for doc in docs for i in range(len(doc)- window_size)]
	...
	print("Shape of concept-concept-edge feature {}".format(mat.shape))
...
def main():
	...
	concepts, concept_to_index, concept_feature = build_concept_feature(concepts, glove_model)
	
	docs = fetch_docs('./MOOC/DSA/data/docs')
	docs, doc_feature = build_doc_feature(docs, concepts, concept_feature)
	
	doc_concept_edge_feature = tfidf(docs, concepts)
